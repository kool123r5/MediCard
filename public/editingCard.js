    ...
    if (user) {
        ...
        db.collection('cardInfo').doc(`${user.email}`).get().then(doc => {
            if (doc.exists) {
                ...
                residencyInput.value =  `${doc.data().residency}`
                mainPhoneInput.value =  `${doc.data().phone}`
                backupPhoneInput.value =  `${doc.data().backupPhone}`
                dobInput.value = `${doc.data().dateOfBirth}`
                var textOriginal = doc.data().importantInfo
                var textCounted = (textOriginal.split("|").length);
                var textArray = textOriginal.split("|")

                for (let numberOfTimes = 0; numberOfTimes < textCounted; numberOfTimes++) {
                    createInputs(textArray[numberOfTimes])
                }
            } else {
                window.location = 'makingCard.html'
            }
        })
